...
browser.storage.sync.get(SETTING_KEYS).then(r => {
...
function loadDefaultDict() {
  return fetch(browser.runtime.getURL("default-dict.txt"))
    .then(r => r.text())
...
function loadCustomDict() {
  return browser.storage.local.get("customDict").then(r => {
    customDict = r.customDict ? parseDict(r.customDict) : null;
  }).catch(() => { customDict = null; });
}
